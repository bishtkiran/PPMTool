...
export const getBacklog = (backlog_id) => async (dispatch) => {
  try {
    const result = await axios.get(`/api/backlog/${backlog_id}`);
    dispatch({
      type: GET_BACKLOG,
      payload: result.data,
    });
...
export const getProjectTask =
  (backlog_id, pt_id, history) => async (dispatch) => {
    try {
      const result = await axios.get(`/api/backlog/${backlog_id}/${pt_id}`);
      dispatch({
...
export const deleteProjectTask = (backlog_id, pt_id) => async (dispatch) => {
  if (
    window.confirm(
      `You are going to delete ${pt_id}, this action can't ne undone.`
    )
  ) {
